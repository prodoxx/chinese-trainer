...
const PROBLEMATIC_PATTERNS: PromptPattern[] = [
  ...
  },
  {
    ...
    replacement: (match) => `${match}. High quality, educational illustration with clear details.`
  }
];
...
export function optimizePrompt(
  ...
  if (!optimized.includes('family-friendly') && !optimized.includes('appropriate')) {
  ...
  if (meaningLower.includes('person') || meaningLower.includes('people')) {
    if (!optimized.includes('full body') && !optimized.includes('three-quarter')) {
  ...
  const confidence = changes.length === 0 ? 1.0 : Math.max(0.7, 1.0 - (changes.length * 0.1));
...
export function generateAlternativePrompts(
  ...
  if (previousIssues.some(issue => issue.includes('complex') || issue.includes('crowd'))) {
    alternatives.push(
  ...
  if (previousIssues.some(issue => issue.includes('anatomy') || issue.includes('anomal'))) {
    alternatives.push(
  ...
  if (previousIssues.some(issue => issue.includes('hands') || issue.includes('feet'))) {
    alternatives.push(
  ...
  if (previousIssues.some(issue => issue.includes('face') || issue.includes('person'))) {
    alternatives.push(
...
export async function optimizeImagePrompt(
  ...
  if (!prompt) {
    const { generateIntelligentPrompt } = await import('./intelligent-prompt-generator');
  ...
  const alternatives = analysis.warnings.length > 0
    ? generateAlternativePrompts(optimization.optimized, hanzi, meaning, analysis.warnings)
    : [];
