...
export default function AdminCardsPage() {
	...
	const [filterStatus, setFilterStatus] = useState<
		"all" | "enriched" | "pending" | "partial"
	>("all");
	const [selectedCards, setSelectedCards] = useState<Set<string>>(new Set());
	const [reEnrichingCards, setReEnrichingCards] = useState<Set<string>>(
		new Set(),
	);
	const [viewMode, setViewMode] = useState<"table" | "grid">("table");
	const [enrichmentStatus, setEnrichmentStatus] = useState<Map<string, string>>(
		new Map(),
	);
	...
	const [sortBy, setSortBy] = useState<"updatedAt" | "createdAt" | "hanzi">(
		"updatedAt",
	);
	...
	const [bulkImportStartTime, setBulkImportStartTime] = useState<number | null>(
		null,
	);
	...
	useEffect(() => {
		...
		if (bulkImporting && bulkImportStartTime) {
			interval = setInterval(() => {
				setBulkImportElapsed(
					Math.floor((Date.now() - bulkImportStartTime) / 1000),
				);
			}, 1000);
		}
	...
	const handleDeleteCard = async () => {
		...
		try {
			const response = await fetch(
				`/api/admin/cards/${deleteConfirmation.cardId}/delete`,
				{
					method: "DELETE",
				},
			);
			...
			if (!response.ok) {
				if (data.deckCount) {
					showAlert(data.message, { type: "error" });
				} else {
	...
	const handleRegenerateAIInsights = async (cardId: string) => {
		try {
			...
			setEnrichmentStatus((prev) =>
				new Map(prev).set(cardId, "Regenerating AI insights..."),
			);

			const response = await fetch(
				`/api/admin/cards/${cardId}/regenerate-ai-insights`,
				{
					method: "POST",
			...
			showAlert(`AI insights regenerated for ${result.card.hanzi}`, {
				type: "success",
			});
			...
			showAlert(
				error instanceof Error
					? error.message
					: "Failed to regenerate AI insights",
				{ type: "error" },
			);
		} finally {
	...
	const handleReEnrichCard = async (cardId: string) => {
		try {
			setReEnrichingCards((prev) => new Set(prev).add(cardId));
			setEnrichmentStatus((prev) =>
				new Map(prev).set(cardId, "Starting enrichment..."),
			);
				...
				try {
					...
					if (jobStatus.progress?.stage) {
						setEnrichmentStatus((prev) =>
							new Map(prev).set(
								cardId,
								jobStatus.progress.message || "Processing...",
							),
						);
					}
					...
					if (terminalStates.includes(jobStatus.state)) {
						...
						if (jobStatus.state === "failed") {
							...
							showAlert(`Re-enrichment failed: ${failureReason}`, {
								type: "error",
							});
						}
					}
	...
	const handleBulkReEnrich = async () => {
		...
		try {
			...
			const data = await response.json();
			showAlert(`Re-enrichment started for ${data.jobCount} cards`, {
				type: "success",
			});
			setSelectedCards(new Set());
			...
			if (data.jobs && data.jobs.length > 0) {
				data.jobs.forEach(
					(job: { jobId: string; cardId: string; hanzi: string }) => {
						// Mark as re-enriching
						setReEnrichingCards((prev) => new Set(prev).add(job.cardId));
						setEnrichmentStatus((prev) =>
							new Map(prev).set(job.cardId, `Enriching ${job.hanzi}...`),
						);
						...
						const pollInterval = setInterval(async () => {
							try {
								const statusResponse = await fetch(
									`/api/admin/cards/job-status?jobId=${job.jobId}`,
								);
								if (!statusResponse.ok)
									throw new Error("Failed to fetch job status");
								...
								if (
									...
									if (jobStatus.state === "completed") {
										...
										console.error(
											`Enrichment failed for ${job.hanzi}:`,
											failureReason,
										);
									}
								}
	...
	const handleBulkImport = async () => {
		...
		try {
			...
			if (data.jobId) {
				...
				showAlert(`Import started for ${data.totalCharacters} characters`, {
					type: "info",
				});
	...
	const updateEnrichmentCount = (increment: boolean = false) => {
		setBulkImportResults((prev: any) => {
			...
			const updated = { ...prev };
			if (increment) {
				updated.summary.completedEnrichment =
					(updated.summary.completedEnrichment || 0) + 1;
				updated.summary.pendingEnrichment = Math.max(
					0,
					(updated.summary.pendingEnrichment || 0) - 1,
				);
			}
			return updated;
		});
	...
	const pollBulkImportJob = (jobId: string) => {
		...
		const poll = async () => {
			...
			try {
				const response = await fetch(
					`/api/admin/cards/bulk-import?jobId=${jobId}`,
				);
				if (!response.ok) {
					throw new Error("Failed to fetch job status");
				}
				...
				if (jobStatus.state === "completed") {
					...
					if (jobStatus.result) {
						...
						const enrichmentCount =
							jobStatus.result.results?.enrichmentJobs?.length || 0;
					...
					setBulkImportJobId(null);
					setBulkImportStartTime(null);
					showAlert("Import timed out. Please check the admin panel later.", {
						type: "warning",
					});
				}
			} catch (error) {
						...
						<div className="flex items-start justify-between">
							<div>
								<div className="flex items-center gap-3 mb-2">
									...
									<h1 className="text-3xl font-bold text-[#f7cc48]">
										All Cards
									</h1>
								</div>
								<p className="text-gray-400">
									Manage all characters in the database
								</p>
							</div>
													...
													<td className="px-6 py-4">
														<div className="flex items-center gap-2">
															...
															{card.audioUrl && (
																...
																>
																	<Volume2
																		className={`w-4 h-4 ${playingAudio === card.audioUrl ? "text-[#f7cc48] animate-pulse" : "text-green-500"}`}
																	/>
																</button>
															)}
									...
									{cards.map((card) => {
										...
										return (
											...
											>
												...
												>
													<div
														className={`font-bold mb-2 ${card.hanzi.length > 2 ? "text-4xl" : "text-6xl"}`}
													>
														{card.hanzi}
													</div>
													<div className="text-lg text-[#f7cc48] mb-1">
														{card.pinyin}
													</div>
													<div className="text-sm text-gray-400 line-clamp-2 px-2">
														{card.meaning}
													</div>
												</div>
												...
												<div className="p-3 border-t border-[#30363d] flex flex-col gap-2">
													<div className="flex gap-2">
														{card.audioUrl ? (
															...
															>
																<Volume2
																	className={`w-4 h-4 ${playingAudio === card.audioUrl ? "text-[#f7cc48]" : "text-gray-400"}`}
																/>
						...
						<div className="mb-6">
							...
							<div className="bg-[#0d1117] border border-[#30363d] rounded-lg p-4">
								...
								<p className="text-sm text-gray-500 text-center">
									This will permanently delete the card and its associated media
									files.
								</p>
							</div>
						</div>
			...
			{bulkDeleteConfirmation && (
				...
				>
					<div className="bg-[#161b22] rounded-2xl p-6 max-w-md w-full border border-[#30363d] shadow-2xl">
						...
						<div className="mb-6">
							<p className="text-gray-300 mb-3">
								Are you sure you want to delete {selectedCards.size} selected
								card{selectedCards.size > 1 ? "s" : ""}?
							</p>
							<div className="bg-[#0d1117] border border-[#30363d] rounded-lg p-4">
								<div className="text-center">
									...
									<p className="text-sm text-gray-500 mt-2">
										This will permanently delete the cards, their character
										analyses, and associated media files.
									</p>
								</div>
							</div>
						</div>
								...
								<div className="space-y-3">
									<div>
										<div className="flex justify-between text-sm text-gray-400 mb-1">
											<span>
												{bulkImportProgress.message || "Processing..."}
											</span>
											<span>
									...
									{bulkImportProgress.batchIndex && (
										<p className="text-sm text-gray-400">
											Batch {bulkImportProgress.batchIndex} of{" "}
											{bulkImportProgress.totalBatches}
										</p>
									)}
...
謝謝"
									...
									<p className="text-xs text-gray-500 mt-2">
										• Traditional Chinese only • One character or word per line
										• Maximum 4 characters per word • Duplicate characters will
										be skipped
									</p>
								</div>
									...
									<div className="grid grid-cols-2 gap-3 text-sm">
										...
										<div>
											...
											<span className="ml-2 font-semibold text-green-400">
												{bulkImportResults.summary.created}
											</span>
										</div>
										<div>
											<span className="text-gray-400">Skipped:</span>
											<span className="ml-2 font-semibold text-yellow-400">
												{bulkImportResults.summary.skipped}
											</span>
										</div>
										<div>
											...
											<div className="col-span-2">
												...
												0 ? (
													<>
														...
														{(bulkImportResults.summary.completedEnrichment ??
															0) > 0 && (
										...
										<h4 className="text-sm font-medium mb-2">
											...
											0 ? (
												...
												<span className="text-green-400">
													Done Importing (
													{bulkImportResults.results.created.length})
												</span>
											)}
										</h4>
										<div className="space-y-1 max-h-32 overflow-y-auto">
											{bulkImportResults.results.created.map(
												(item: any, idx: number) => (
													<div
														key={idx}
								...
								{bulkImportResults.results.skipped.length > 0 && (
									<div>
										<h4 className="text-sm font-medium text-yellow-400 mb-2">
											Skipped ({bulkImportResults.results.skipped.length})
										</h4>
										<div className="space-y-1 max-h-32 overflow-y-auto">
											{bulkImportResults.results.skipped.map(
												(item: any, idx: number) => (
								...
								{bulkImportResults.results.errors.length > 0 && (
									<div>
										<h4 className="text-sm font-medium text-red-400 mb-2">
											Errors ({bulkImportResults.results.errors.length})
										</h4>
										<div className="space-y-1 max-h-32 overflow-y-auto">
											{bulkImportResults.results.errors.map(
												(item: any, idx: number) => (
													<div key={idx} className="text-sm text-gray-300">
														<span className="font-semibold">{item.hanzi}</span>
					...
					<div className="bg-[#161b22] rounded-2xl p-6 max-w-4xl w-full border border-[#30363d] shadow-2xl max-h-[90vh] overflow-y-auto">
						...
						<div className="mt-6">
							...
							<p className="text-xs text-gray-400 mt-2">
								Tip: Be specific about style, composition, and visual elements.
								The AI will create an illustration based on this prompt.
							</p>
						</div>
							...
							<div className="flex gap-3">
								<button
									onClick={async () => {
										...
										try {
											const response = await fetch(
												"/api/admin/cards/generate-image",
												{
													method: "POST",
													headers: { "Content-Type": "application/json" },
													body: JSON.stringify({
											...
											if (data.success && data.imageUrl) {
												...
												showAlert("Preview image generated!", {
													type: "success",
												});
											} else {
										...
										onClick={async () => {
											...
											try {
												...
												if (data.success) {
													...
													showAlert(data.error || "Failed to save image", {
														type: "error",
													});
												}
											} catch (error) {
