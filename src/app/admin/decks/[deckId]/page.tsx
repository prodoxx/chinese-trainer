...
export default function AdminDeckPage({ params }: { params: Promise<{ deckId: string }> }) {
  ...
  const handleReEnrichCard = async (cardId: string) => {
    try {
      ...
      showAlert(`Re-enrichment started for card "${deck?.cards.find(c => c._id === cardId)?.hanzi}"`, { type: 'success' })
      
      // Refresh after a delay to see status update
                    ...
                    <tbody className="divide-y divide-[#30363d]">
                      {filteredCards.map((card) => (
                        <tr key={card._id} className="hover:bg-[#0d1117] transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className="text-2xl font-bold text-[#f7cc48]">{card.hanzi}</span>
                          </td>
