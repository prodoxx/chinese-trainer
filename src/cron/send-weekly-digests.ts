...
async function sendWeeklyDigests() {
	...
	console.log(
		`[${new Date().toISOString()}] Starting weekly digest distribution`,
	);
	...
	try {
		...
		console.log(
			`Found ${usersWithPrefs.length} users subscribed to weekly digest`,
		);
		...
		for (const prefs of usersWithPrefs) {
			try {
				...
				if (!lastSent || lastSent < oneWeekAgo) {
					...
					console.log(
						`Scheduled weekly digest for user ${prefs.userId} (job ${job.id})`,
					);
					scheduledCount++;
				} else {
		...
		console.log(
			`Completed: ${scheduledCount} scheduled, ${skippedCount} skipped, took ${duration}ms`,
		);
