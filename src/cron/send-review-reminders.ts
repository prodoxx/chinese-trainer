...
async function sendHourlyReminders() {
	...
	console.log(
		`[${new Date().toISOString()}] Starting hourly reminder check for UTC hour ${currentUtcHour}`,
	);
			...
			try {
				...
				if (userCurrentHour === prefHour) {
					...
					if (!lastSent || lastSent < today) {
						...
						console.log(
							`Scheduled reminder for user ${prefs.userId} (job ${job.id}) - ${prefs.reminderTime} ${userTimeZone}`,
						);
						scheduledCount++;
					} else {
